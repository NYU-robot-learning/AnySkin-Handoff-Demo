...
class Dataset:
    def __init__(self, path: str, window_len: int, pull_len: int, normalize_max: float):
        ...
        for seq, label in zip(self.seqs, self.labels):
            ...
            assert baseline.shape == (1, 15)

            start_range = max(label - pull_len, 0)
            end_range = min(label + pull_len, len(seq))

            for i in range(start_range, end_range):
                if label - 5 < i < label + 5: # buffer zone
                    continue
                features = seq[i : i + window_len, 15:] - baseline
                features = features[:, xy_mask]
                samples.append((features/normalize_max, int(i >= label)))
